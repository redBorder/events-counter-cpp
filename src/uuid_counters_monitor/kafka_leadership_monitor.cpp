...
#include "kafka_leadership_monitor.hpp"
...
KafkaLeadershipMonitor::KafkaLeadershipMonitor(const string &brokers,
                                               const string &topic,
                                               const string &group) {
...
KafkaLeadershipMonitor::~KafkaLeadershipMonitor() {
  this->kafka_consumer->close();
}

bool KafkaLeadershipMonitor::check_leadership(
    chrono::milliseconds timeout) const {
  do {
    unique_ptr<Message> message(this->kafka_consumer->consume(timeout.count()));
...
void KafkaLeadershipMonitor::rebalance_cb(
    KafkaConsumer *, ErrorCode err, vector<TopicPartition *> &partitions) {

  this->is_leader = err == ERR__ASSIGN_PARTITIONS && partitions.size() > 0;
}
